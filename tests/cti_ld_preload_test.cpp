...
int main(int argc, char* argv[])
{
    ...
    { // Launch application without preload, expect response of 1
        // set up app
        auto const  appArgv = createSystemArgv(argc, argv, {oneSocketPath, address, port}); auto const  stdoutFd = -1;
    ...
    { // Launch application with preload, expect response of 2
        // set up app
        auto const  appArgv = createSystemArgv(argc, argv, {oneSocketPath.c_str(), address.c_str(), port.c_str()});
